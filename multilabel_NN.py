...
if what_data_use == 'rna':
	data = np.column_stack((label,rna_data))
	feature = rna_feature
else:
	if what_data_use == 'beta':
	        data = np.column_stack((label,beta_data))
		feature = beta_feature
	else: 
		data = np.column_stack((label,rna_data,beta_data))
		feature = rna_feature + beta_feature

print("# of samples:", len(samples))
print("Dimension of data:", data.shape)
print("# of MSDP:", len(np.argwhere(label==2))
print("# of SDPC:", len(np.argwhere(label==0))
print("# of SDP:", len(np.argwhere(label==1))
...
if l2_regularizer_use == 1:
	loss = tf.nn.softmax_cross_entropy_with_logits(labels=y,logits=y_) + l2_regularization
else: loss = tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=y_)
